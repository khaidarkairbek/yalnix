...
extern void handle_trap_memory(UserContext *uctx) {
  ...
  */
  ...
  int address = (int) uctx->addr;
  if (address < VMEM_1_BASE || address > VMEM_1_LIMIT) {
    pcb_terminate(g_current_process, ERROR);
    *uctx = g_current_process->uctx;
    return;
  }
  ...
  // Stack growth
  int new_low_page = address >> PAGESHIFT;
  int old_low_page = g_current_process->ustack_low >> PAGESHIFT; 
  ...
  for (int vpn = new_low_page; vpn < old_low_page; vpn++) {
    int region_1_index = vpn - (VMEM_1_BASE >> PAGESHIFT);
    int frame = frame_alloc();
    if (frame == -1){
      TracePrintf(0, "handle_trap_memory: unexpected malloc failed\n"); 
      Halt(); 
    }
...
extern void handle_trap_tty_receive(UserContext *uctx) {
  ...
  */
  ...
  int tty_id = uctx->code; 
  void *buf = malloc(TERMINAL_MAX_LINE); 
  if (buf == NULL){
    // TODO: figure out what to do
    TracePrintf(0, "handle_trap_tty_receive: malloc failed\n"); 
    *uctx = g_current_process->uctx;
    return; 
  }
